...
public class EnemyDronAI : MonoBehaviour
{
    ...
    private EnemyDronHealth enemyDronHealth;
    ...
    private void Awake()
    {
        ...
        enemyDronHealth = GetComponent<EnemyDronHealth>();
    ...
    private void Update()
    {
        if (enemyDronHealth.IsDead) return;//죽으면 반복문을 멈춤
        ...
        if (targetEntity != null)
        {
            ...
            if (distance <= attackDistance && enemyDronHealth.IsDead == false)
            {
                //폭발 명령
                enemyDronHealth.Die(0);
            }
        }

        if (isTarget && enemyDronHealth.IsDead == false)
        {
    ...
    public void ExplosionAttack()
    {
        ...
        foreach (RaycastHit hitobj in rayHits)
        {
            ...
            {
                Debug.Log("공격 성공");
                hitobj.transform.GetComponent<LivingEntity>().IsApplyDamage(damage,enemyDronHealth.livingEntity);
            }
